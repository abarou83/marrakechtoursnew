                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                @if($activePromo)
                                    @php
                                        $fmtMin = \App\Helpers\CurrencyHelper::format(\App\Helpers\CurrencyHelper::convert((float)$minPrice));
                                        $fmtFinal = \App\Helpers\CurrencyHelper::format(\App\Helpers\CurrencyHelper::convert((float)$finalPrice));
                                    @endphp
                                    <span class="text-gray-400 line-through text-sm">{{ $fmtMin }}</span>
                                    <span class="text-2xl font-bold text-gray-900 ml-2">{{ $fmtFinal }}</span>
                                @else
                                    @php $fmtMinOnly = \App\Helpers\CurrencyHelper::format(\App\Helpers\CurrencyHelper::convert((float)$minPrice)); @endphp
                                    <span class="text-2xl font-bold text-gray-900">{{ $fmtMinOnly }}</span>
                                @endif
