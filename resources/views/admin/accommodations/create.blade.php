...
@section('title', 'Créer un Hébergement')

@section('content')
<div class="mb-6">
    <a href="{{ route('admin.accommodations.index') }}" class="text-blue-600 hover:underline">← Retour aux Hébergements</a>
    <h2 class="text-2xl font-bold text-gray-900 mt-2">Créer un Nouvel Hébergement</h2>
</div>
...
<form action="{{ route('admin.accommodations.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 max-w-4xl">
    @csrf
    @php
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            <p class="mt-1 text-sm text-gray-500">ex: "Hôtel Riad Al Kasbah", "Résidence El Andalous"</p>
        </div>

        <div>
            <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">Slug</label>
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
            <p class="mt-1 text-sm text-gray-500">Laisser vide pour générer automatiquement</p>
        </div>

        <div>
            <label for="location" class="block text-sm font-medium text-gray-700 mb-2">Localisation</label>
            <input type="text" name="location" id="location" value="{{ old('location') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="ex: Marrakech, Casablanca">
        </div>
        ...
        <div>
            ...
            <select name="stars" id="stars" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                @for($i = 1; $i <= 5; $i++)
                    <option value="{{ $i }}" {{ old('stars') == $i ? 'selected' : '' }}>{{ $i }} {{ $i > 1 ? 'étoiles' : 'étoile' }}</option>
                @endfor
            </select>
        </div>
        ...
        <div class="md:col-span-2">
            ...
            <input type="text" name="address" id="address" value="{{ old('address') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="ex: Avenue Mohamed VI, Guéliz, 40000 Marrakech">
        </div>

        <div class="md:col-span-2">
            <label for="description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
            <textarea name="description" id="description" rows="3" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Description de l'hébergement...">{{ old('description') }}</textarea>
        </div>
    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="translations[{{ $loop->index }}][description]" rows="2"
                                      class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old("translations.{$loop->index}.description") }}</textarea>
                        </div>
                        <div>
                            ...
                            <input type="text" name="translations[{{ $loop->index }}][location]"
                                   value="{{ old("translations.{$loop->index}.location") }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ajouter une première chambre par défaut
    createRoomField();
});
</script>
@endsection
