...
<div class="mb-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
    <h2 class="text-lg font-semibold text-gray-900 mb-1">Ajouter un avis</h2>
    <p class="text-sm text-gray-500">Créez un nouvel avis pour un tour.</p>
</div>

<form action="{{ route('admin.reviews.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 border border-gray-200">
    @csrf
    ...
    <div class="mb-6">
        ...
        <textarea name="comment" id="comment" rows="5" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Laissez un commentaire sur ce tour...">{{ old('comment') }}</textarea>
        @error('comment')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-6">
        <label class="flex items-center">
            <input type="checkbox" name="is_approved" value="1" {{ old('is_approved') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            <span class="ml-2 text-sm text-gray-700">Approuver cet avis immédiatement</span>
        </label>
    </div>
