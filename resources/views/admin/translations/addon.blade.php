...
@section('title', 'Traductions de l\'Addon')

@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.addons.index') }}" class="text-indigo-600 hover:text-indigo-800 font-semibold">
            ← Retour aux addons
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg max-w-4xl" x-data="{ activeTab: '{{ $availableLocales[0] ?? 'fr' }}' }">
        <div class="p-8 pb-0">
            <h2 class="text-2xl font-bold text-gray-900 mb-2"><i class="fas fa-language mr-2"></i>Traductions : {{ $addon->name }}</h2>
            <p class="text-gray-600 mb-6">Gérez les traductions de cet addon dans toutes les langues supportées</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.addons.translations.update', $addon) }}">
            @csrf
                    ...
                    <div x-show="activeTab === '{{ $locale }}'" x-transition>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">
                                Nom de l'addon <span class="text-red-500">*</span>
                            </label>
                            <input type="text" 
