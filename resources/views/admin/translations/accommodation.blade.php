...
@section('title', 'Traductions de l\'Hébergement')

@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.accommodations.index') }}" class="text-indigo-600 hover:text-indigo-800 font-semibold">
    ...
    <div class="bg-white rounded-xl shadow-lg max-w-4xl" x-data="{ activeTab: '{{ $availableLocales[0] ?? 'fr' }}' }">
        <div class="p-8 pb-0">
            <h2 class="text-2xl font-bold text-gray-900 mb-2"><i class="fas fa-language mr-2"></i>Traductions : {{ $accommodation->name }}</h2>
            <p class="text-gray-600 mb-6">Gérez les traductions de cet hébergement dans toutes les langues supportées</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.accommodations.translations.update', $accommodation) }}">
            @csrf
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" 
                                       ...
                                       required
                                       placeholder="Nom en {{ strtolower($localeInfo['native']) }}">
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="translations[{{ $loop->index }}][description]" 
                                          rows="3"
                                          class="w-full border-gray-300 rounded-lg px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Description en {{ strtolower($localeInfo['native']) }}">{{ old("translations.{$loop->index}.description", $translation->description ?? $accommodation->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       ...
                                       class="w-full border-gray-300 rounded-lg px-4 py-3 focus:border-indigo-500 focus:ring-indigo-500"
                                       placeholder="Localisation en {{ strtolower($localeInfo['native']) }}">
                            </div>
                        </div>
                    </div>
                @endforeach
