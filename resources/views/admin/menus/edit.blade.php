...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-lg p-8 max-w-6xl">
        <div class="mb-8 pb-6 border-b border-gray-200">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Modifier le Menu</h1>
            <p class="text-gray-600">Modifiez les informations et les items de votre menu de navigation</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.menus.update', $menu) }}" id="menu-form">
            ...
            <div class="mb-8 bg-gray-50 rounded-lg p-6 border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="location" id="menu-location"
                            ...
                            @foreach(\App\Models\Menu::locationLabels() as $value => $label)
                                <option value="{{ $value }}" {{ old('location', $menu->location ?? 'header') === $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="position" value="{{ old('position', $menu->position) }}" 
                               class="w-full border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-primary"
                               min="0" placeholder="0">
            ...
            <div id="menu-title-translations" class="mb-8 bg-gray-50 rounded-lg p-6 border border-gray-200 hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($locales as $locale => $localeInfo)
                        @php
                            $menuTranslation = $menu->translations->where('locale', $locale)->first();
                        @endphp
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old("menu_translations.$locale.name", $menuTranslation->name ?? '') }}"
                                   class="w-full border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-primary"
            ...
            <div class="mb-8 bg-gray-50 rounded-lg border border-gray-200 p-6">
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Gérez les liens de navigation (multilingues)</p>
                        </div>
                    </div>
                ...
                <div id="menu-items-container" class="space-y-4">
                    @foreach($menu->allItems->sortBy('order') as $index => $item)
                        @php
                            $itemLinkType = in_array($item->link_type, ['internal', 'external']) ? 'custom' : $item->link_type;
                            $itemLinkTypeLabels = ['custom' => 'Personnalisé', 'category' => 'Catégorie', 'page' => 'Page', 'tour' => 'Tour'];
                        @endphp
                        <div class="menu-item bg-white border-2 border-gray-200 rounded-lg hover:border-primary/50 transition-all" data-index="{{ $index }}" x-data="{ isOpen: false }">
                            <div class="flex justify-between items-center p-4 border-b border-gray-200">
                                <div class="flex items-center space-x-3 flex-1">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-gray-900 item-display-name">{{ $item->getDisplayLabel() ?: 'Item #' . ($index + 1) }}</h4>
                                        <p class="text-xs text-gray-500 flex items-center mt-1 item-display-type">
                                            {{ $itemLinkTypeLabels[$itemLinkType] ?? $itemLinkType }}
                                        </p>
                                    </div>
                            ...
                            <div x-show="isOpen" x-transition class="p-6">
                                ...
                                <div>
                                    ...
                                    <select name="items[{{ $index }}][link_type]" 
                                        ...
                                        <option value="custom" {{ $itemLinkType === 'custom' ? 'selected' : '' }}>Personnalisé (URL)</option>
                                        <option value="category" {{ $itemLinkType === 'category' ? 'selected' : '' }}>Catégorie</option>
                                        <option value="page" {{ $itemLinkType === 'page' ? 'selected' : '' }}>Page</option>
                                        <option value="tour" {{ $itemLinkType === 'tour' ? 'selected' : '' }}>Tour</option>
                                    </select>
                                </div>
                                
                                @php $isEntityItem = in_array($itemLinkType, ['category', 'page', 'tour']); @endphp
                                <div class="item-translations-section border border-gray-200 rounded-lg bg-white">
                                    <div class="border-b border-gray-200 p-3 bg-gray-50 rounded-t-lg">
                                        <label class="item-translations-label block text-xs font-semibold text-gray-600 mb-2">
                                            Label Multilingue
                                            <span class="text-red-500 translation-required-star {{ $isEntityItem ? 'hidden' : '' }}">*</span>
                                        </label>
                                        <p class="item-translations-hint text-xs text-blue-600 {{ $isEntityItem ? '' : 'hidden' }}">
                                            <i class="fas fa-info-circle mr-1"></i>Le label est récupéré automatiquement depuis la catégorie, la page ou le tour sélectionné.
                                        </p>
                                    </div>
                                    <div class="p-3 item-translations-inputs space-y-3 {{ $isEntityItem ? 'hidden' : '' }}">
                                        @foreach($availableLocales as $localeIndex => $locale)
                                            @php
                                                $itemTranslation = $item->translations->where('locale', $locale)->first();
                                            @endphp
                                            <div>
                                                <label class="block text-xs text-gray-500 mb-1">{{ strtoupper($locale) }}</label>
                                                <input type="hidden" name="items[{{ $index }}][translations][{{ $localeIndex }}][locale]" value="{{ $locale }}">
                                                <input type="text" 
                                                       name="items[{{ $index }}][translations][{{ $localeIndex }}][label]" 
                                                       value="{{ old("items.$index.translations.$localeIndex.label", $isEntityItem ? '' : ($itemTranslation->label ?? '')) }}"
                                                       class="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary item-translation-input" 
                                                       data-item-index="{{ $index }}"
                                                       data-locale="{{ $locale }}"
                                                       placeholder="Label en {{ $locale }}"
                                                       {{ $isEntityItem ? 'disabled' : '' }}>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <div class="link-fields mb-4" id="link-fields-{{ $index }}">
                                <div class="custom-link-fields" style="display: {{ $isEntityItem ? 'none' : 'block' }};">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                                ...
                                <div class="category-field" style="display: {{ $itemLinkType === 'category' ? 'block' : 'none' }};">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                                ...
                                <div class="page-field" style="display: {{ $itemLinkType === 'page' ? 'block' : 'none' }};">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                                    ...
                                    <select name="items[{{ $index }}][page_id]" 
                                        ...
                                        @foreach($pages as $page)
                                            <option value="{{ $page->id }}" {{ $item->page_id == $page->id ? 'selected' : '' }}>
                                                {{ translate_model($page, 'title') }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>

                                <div class="tour-field" style="display: {{ $itemLinkType === 'tour' ? 'block' : 'none' }};">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                                    ...
                                    <select name="items[{{ $index }}][tour_id]" 
                                        ...
                                        @foreach($tours as $tour)
                                            <option value="{{ $tour->id }}" {{ $item->tour_id == $tour->id ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-200">
                                <div>
                                    ...
                                    <input type="number" name="items[{{ $index }}][order]" 
                                           value="{{ old("items.$index.order", $item->order ?? $index) }}" 
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                ...
                <button type="submit" 
                        class="inline-flex items-center px-8 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 font-bold shadow-lg hover:shadow-xl transition-all">
    ...
    <script>
        let itemIndex = {{ $menu->allItems->count() }};
                ...
                <div class="menu-item bg-white border-2 border-gray-200 rounded-lg hover:border-primary/50 transition-all" data-index="${index}" x-data="{ isOpen: true }">
                    <div class="flex justify-between items-center p-4 border-b border-gray-200">
                        <div class="flex items-center space-x-3 flex-1">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-900 item-display-name">Item #${index + 1}</h4>
