...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-900 mt-2">Create New Addon</h2>
</div>
...
<form action="{{ route('admin.addons.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 max-w-2xl">
    @csrf
    ...
    @if(request()->has('tour_id'))
        <input type="hidden" name="tour_id" value="{{ request()->query('tour_id') }}">
    @endif
    ...
    <div class="mb-6">
        ...
        <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
        <p class="mt-1 text-sm text-gray-500">e.g., "Lunch", "Private Guide", "Camel Ride"</p>
    </div>
    ...
    <div class="mb-6">
        ...
        <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <div class="mb-6">
        ...
        <select name="pricing_type" id="pricing_type" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
            <option value="per_person" {{ old('pricing_type') === 'per_person' ? 'selected' : '' }}>Per Person</option>
            <option value="per_group" {{ old('pricing_type') === 'per_group' ? 'selected' : '' }}>Per Group</option>
    ...
    <div class="mb-6" id="base-price-section">
        ...
        <input type="number" name="base_price" id="base_price" value="{{ old('base_price') }}" step="0.01" min="0" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <p class="mt-1 text-sm text-gray-500">Used if no price tiers are configured. Can be overridden per tour.</p>
    </div>

    <!-- Price Tiers Section (for per_person addons) -->
    <div class="mb-6" id="price-tiers-section" style="display: {{ old('pricing_type') === 'per_person' ? 'block' : 'none' }};">
