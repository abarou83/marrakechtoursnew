        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="mb-6">
                    ...
                    <h3 class="text-2xl font-bold text-gray-800 mt-2"><i class="fas fa-tag mr-2"></i>Nouvelle promotion pour : {{ $tour->title }}</h3>
                </div>

                <form method="POST" action="{{ route('admin.tour-promotions.store', $tour) }}">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="3" 
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                  placeholder="Détails de l'offre...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select name="discount_type" id="discount_type" 
                                ...
                                <option value="fixed" {{ old('discount_type') == 'fixed' ? 'selected' : '' }}>Montant fixe (EUR)</option>
                            </select>
                            @error('discount_type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select name="badge_color" id="badge_color" 
                                ...
                                <option value="red" {{ old('badge_color') == 'red' ? 'selected' : '' }}>🔴 Rouge</option>
                                <option value="orange" {{ old('badge_color') == 'orange' ? 'selected' : '' }}>🟠 Orange</option>
                                <option value="yellow" {{ old('badge_color') == 'yellow' ? 'selected' : '' }}>🟡 Jaune</option>
                                <option value="green" {{ old('badge_color') == 'green' ? 'selected' : '' }}>🟢 Vert</option>
                                <option value="blue" {{ old('badge_color') == 'blue' ? 'selected' : '' }}>🔵 Bleu</option>
                                <option value="purple" {{ old('badge_color') == 'purple' ? 'selected' : '' }}>🟣 Violet</option>
                                <option value="pink" {{ old('badge_color') == 'pink' ? 'selected' : '' }}>💗 Rose</option>
                            </select>
                            @error('badge_color')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex items-center justify-end space-x-4">
                        ...
                        <button type="submit" 
                            ...
                            <i class="fas fa-plus mr-2"></i>Créer la promotion
                        </button>
                    </div>
                </form>
            </div>
